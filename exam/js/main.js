...
(function() {
    ...
    const CONFIG = {
        examDuration: 2 * 60 * 60 * 1000, // 2 hours in ms
        accessPassword: 'Exam2026!',
        ...
        crossSectionImagePath: 'images/exam_crosssection_new.png',
    ...
    function cacheDOMElements() {
        ...
        DOM.accessPasswordInput = document.getElementById('accessPassword');
    ...
    async function initializeCanvases() {
        ...
        // Wheeler diagram should match cross-section width and have extended height
        // to accommodate 50 time units clearly with more vertical space
    ...
    function handleLogin(e) {
        ...
        const accessPassword = DOM.accessPasswordInput ? DOM.accessPasswordInput.value : '';
        ...
        if (accessPassword !== CONFIG.accessPassword) {
            showToast('Invalid access password. Please contact your instructor.', 'error');
            if (DOM.accessPasswordInput) {
                DOM.accessPasswordInput.value = '';
                DOM.accessPasswordInput.focus();
            }
            return;
        }
     ...
     * Projects a vertical line to the Wheeler diagram at the same X position
     */
    function handleDrillingLinePlaced(drillData) {
        if (!AppState.wheelerCanvas) return;
        ...
        const xRatio = drillData.x / drillData.canvasWidth;

        // Map to Wheeler diagram X coordinate (within the plot area)
        const plotWidth = wheelerCanvas.width - padding.left - padding.right;
        const wheelerX = padding.left + (xRatio * plotWidth);

        // Add a vertical line spanning the full plot height on Wheeler diagram
        const drillingProjection = {
            type: 'drillingProjection',
            color: '#8B4513',  // Brown color for drilling
            x: wheelerX,
            y1: padding.top,
            y2: wheelerCanvas.height - padding.bottom,
            label: drillData.label,
            lineWidth: 2,
            timestamp: Date.now()
        };

        AppState.wheelerCanvas.addElement(drillingProjection);
        showToast(`Drilling ${drillData.label} projected to Wheeler diagram`, 'success', 2000);
    }

    /**
     * Handle auto-generation of system tract polygon from termination lines
     */
    function handleGenerateAutoTract() {
        ...
        if (isNaN(fromNum) || isNaN(toNum) || fromNum > toNum) {
            showToast('Invalid termination range', 'error');
            return;
        }

        // Find termination lines in Wheeler canvas within range
        ...
        if (terminationLines.length < 2) {
            showToast('Need at least 2 termination lines in range', 'error');
            return;
        }

        // Sort by number (time order)
        terminationLines.sort((a, b) => a.number - b.number);

        // Build polygon points from endpoints
        // Go up the left side (x1 values), then down the right side (x2 values)
        const polygonPoints = [];

        // Left side - going up (from first to last)
        for (const line of terminationLines) {
            polygonPoints.push({ x: line.x1, y: line.y });
        }

        // Right side - going down (from last to first)
        ...
        const tractColors = {
            ...
            'TST': '#00FFFF',   // Cyan
            'LST': '#FFA500',   // Orange
            'FSST': '#FF69B4',  // Hot Pink
            'RST': '#90EE90'    // Light Green
        };

        // Create the system tract element
        const tractElement = {
            type: 'systemTract',
            tractType: tractType,
            color: tractColors[tractType] || '#CCCCCC',
