...
class DrawingCanvas {
    ...
    handleMouseDown(e) {
        ...
        switch (this.currentTool) {
            ...
            case 'termination':
                ...
                break;
            case 'strataNumber':
                this.placeStrataNumber(pos);
                break;
            case 'drilling':
                this.placeDrillingSite(pos);
                break;
            case 'surface':
    ...
    cancelCurrentOperation() {
        ...
        // Cancel any in-progress drilling
        this.cancelDrilling();
    }
    ...
    /**
     * Place a numbered strata marker (single click, auto-increment)
     */
    ...
    /**
     * Initialize drilling state for two-click workflow
     */
    initDrillingState() {
        if (!this.drillingState) {
            this.drillingState = {
                waitingForSecondClick: false,
                rigPosition: null
            };
    ...
    /**
     * Place a drilling site marker (two-click workflow)
     * First click: place drilling rig at surface
     * Second click: place target reservoir and draw connecting line
     */
    placeDrillingSite(pos) {
        this.initDrillingState();

        if (!this.drillingState.waitingForSecondClick) {
            // FIRST CLICK - Place drilling rig
            this.drillingState = {
                waitingForSecondClick: true,
                rigPosition: { x: pos.x, y: pos.y }
            };

            // Create drilling rig element
            const rigElement = {
                type: 'drillingSite',
                color: '#FF0000',
                x: pos.x,
            ...
            // Show hint for second click
            this.showDrillingHint('Click on target reservoir interval');

        } else {
            // SECOND CLICK - Draw line to reservoir target
            const rigPos = this.drillingState.rigPosition;

            // Create drilling line from rig to target
            const lineElement = {
                type: 'drillingLine',
                color: '#FF0000',
                x1: rigPos.x,
            ...
            // Trigger callback to project to Wheeler diagram
            this.onDrillingLinePlaced({
                x: rigPos.x,  // Use rig X position for projection
                y1: rigPos.y,
                y2: pos.y,
                canvasWidth: this.canvas.width,
                canvasHeight: this.canvas.height
    ...
    /**
     * Show hint message for drilling tool
     */
    ...
    /**
     * Cancel drilling if in progress
     */
    cancelDrilling() {
        if (this.drillingState && this.drillingState.waitingForSecondClick) {
            // Remove the rig that was placed
            const lastElement = this.elements[this.elements.length - 1];
            if (lastElement && lastElement.type === 'drillingSite') {
                this.elements.pop();
            }
    ...
    isPointInElement(pos, element) {
        ...
        switch (element.type) {
            ...
            case 'systemTract':
                return this.isPointInPolygon(pos, element.points);
            case 'strataNumber':
                return Math.hypot(pos.x - element.x, pos.y - element.y) < 10;
            case 'drillingSite':
                return Math.hypot(pos.x - element.x, pos.y - element.y) < 20;
            case 'drillingLine':
                return this.isPointNearLine(pos, [
                    { x: element.x1, y: element.y1 },
                    { x: element.x2, y: element.y2 }
                ], threshold);
            case 'drillingProjection':
                return Math.abs(pos.x - element.x) < 8 &&
                       pos.y >= element.y1 && pos.y <= element.y2;
            default:
                return false;
        }
    }
    ...
    drawElement(element) {
        switch (element.type) {
            ...
            case 'rolloverSquare':
                ...
                break;
            case 'strataNumber':
                this.drawStrataNumber(element);
                break;
            case 'drillingSite':
                this.drawDrillingSite(element);
                break;
            case 'drillingLine':
                this.drawDrillingLine(element);
                break;
            case 'drillingProjection':
                this.drawDrillingProjection(element);
                break;
        }
    }

    /**
     * Draw strata number marker (circled number) - compact size
     */
    ...
    /**
     * Draw drilling site marker (oil derrick icon)
     */
    drawDrillingSite(element) {
        ...
        // Triangle tower
        this.ctx.moveTo(x, y - 25);      // Top
        this.ctx.lineTo(x - 12, y + 10); // Bottom left
    ...
    /**
     * Draw drilling line (from rig to reservoir target on cross-section)
     */
    drawDrillingLine(element) {
        const ctx = this.ctx;

        // Draw dashed line from rig to target
        ctx.beginPath();
        ...
        // Draw small target circle at reservoir
        ctx.beginPath();
    ...
    /**
     * Draw drilling projection (subtle vertical line on Wheeler diagram)
     */
    drawDrillingProjection(element) {
        const ctx = this.ctx;

        // Draw subtle dashed vertical line
        ctx.beginPath();
        ctx.strokeStyle = element.color || 'rgba(255, 0, 0, 0.4)';
        ctx.lineWidth = 1.5;
        ctx.setLineDash([4, 4]);
        ctx.moveTo(element.x, element.y1);
        ...
        ctx.fillText('⛏', element.x, element.y1 - 2);
    }
    ...
    handleTouchStart(e) {
        ...
        // Double-tap detection (for finishing lines)
        if (this.lastTapTime && (now - this.lastTapTime) < 300 &&
            this.lastTapPos &&
            Math.abs(touch.clientX - this.lastTapPos.x) < 30 &&
            Math.abs(touch.clientY - this.lastTapPos.y) < 30) {
            // Double tap detected
